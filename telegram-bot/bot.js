...
*Account Linking:*
🔐 *Email \\+ Telegram Key* \\- Use your email and permanent key \\(recommended\\)
🔢 *Generate Code* \\- Create a temporary code for the app
⌨️ *Enter Code* \\- Type a temporary code from the app
...
🔑 Your telegram key is permanent and found in app settings
...
// Handle text messages (for email and telegram key input)
bot.on('message', async (msg) => {
    ...
    if (userSessions.get(`${userId}_awaiting_email`)) {
        ...
        // Store email and ask for telegram key
        userSessions.set(`${userId}_email`, email);
        userSessions.delete(`${userId}_awaiting_email`);
        userSessions.set(`${userId}_awaiting_telegram_key`, true);
        
        const message = `
✅ *Email received:* ${email}

🔑 *Now enter your Telegram Key*

Please enter your permanent telegram key from the Kita\\-kita app:

💡 *Format:* TG\\-XXXXX\\-XXXXX

⌨️ *Waiting for your telegram key\\.\\.\\.*
        `;
    ...
    // Check if user is awaiting telegram key input
    if (userSessions.get(`${userId}_awaiting_telegram_key`)) {
        const telegramKey = text.trim().toUpperCase();
        
        // Validate telegram key format (updated for server-side format)
        if (!/^TG-[A-Z0-9]+(-[A-Z0-9]+)*$/.test(telegramKey)) {
            await bot.sendMessage(chatId, '❌ Please enter a valid telegram key format: TG-XXXXXX-XXXXXXX-XXXXXX');
            return;
        }
        
        const email = userSessions.get(`${userId}_email`);
        await processEmailAndTelegramKey(chatId, userId, email, telegramKey);
        return;
    }
    
    // Check if user is awaiting link code (for old method)
    if (userSessions.get(`${userId}_awaiting_code`)) {
        const code = text.trim().toUpperCase();
        
        // Handle regular 6-character link code (old method)
        if (/^[A-Z0-9]{6}$/.test(code)) {
            await processLinkCode(chatId, userId, code);
        } else {
            await bot.sendMessage(chatId, '❌ Please enter a valid 6-character code.');
        }
        return;
    }
    
    // If no specific context, provide general help
    await bot.sendMessage(chatId, '❓ I didn\'t understand that. Send /help for available commands or use /link to connect your account!');
});
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        switch (data) {
            case 'scan_receipt':
                await bot.sendMessage(chatId, '📸 Please send me a photo of your receipt and I\'ll extract the transaction details for you!');
                break;
            ...
            case 'show_help':
                await bot.sendMessage(chatId, await getHelpMessage(), { parse_mode: 'MarkdownV2' });
                break;
            
            case 'connect_email_key':
                await handleConnectEmailKey(chatId, userId);
                break;
            ...
            case 'link_help':
                await handleLinkHelp(chatId, userId);
                break;
...
// Handle link help
async function handleLinkHelp(chatId, userId) {
    const helpMessage = `
🔗 *Account Linking Help*

*Recommended Method: Email \\+ Telegram Key*

**Step 1:** Find your permanent telegram key
• Open Kita\\-kita app
• Go to Settings \\> Telegram Integration
• Copy your key \\(format: \`TG\\-XXXXX\\-XXXXX\`\\)

**Step 2:** Connect in bot
• Click "Connect with Email \\+ Key"
• Enter your email address
• Enter your telegram key
• Account automatically linked\\!

*Alternative Methods:*

**Temporary Codes \\(old method\\)**
• Generate 6\\-character code in bot or app
• Enter code in opposite platform
• Valid for limited time only

*Why use Email \\+ Key?*
✅ **Permanent** \\- No expiration
✅ **Secure** \\- Your personal credentials  
✅ **Simple** \\- Two\\-step process
✅ **Reliable** \\- Always works

*Can't find your telegram key?*
📱 Check app Settings \\> Telegram Integration
🔄 You can regenerate it if needed

*Still having trouble?*
Contact support in the Kita\\-kita app\\!
    `;
...
To connect your account, I need two things:

**1\\. Your Email Address** \\(used in Kita\\-kita app\\)
**2\\. Your Personal Telegram Key** \\(from your account\\)

📱 *Where to find your Telegram Key:*
• Open Kita\\-kita app \\> Settings \\> Telegram Integration
• Copy your permanent key \\(format: TG\\-XXXXX\\-XXXXX\\)

🚀 *Ready to connect?*
Click the button below to start the process\\!

💡 *Alternative:* You can also use temporary link codes if preferred\\.
    `;
    
    const keyboard = {
        inline_keyboard: [
            [
                { text: '🔐 Connect with Email + Key', callback_data: 'connect_email_key' }
            ],
            [
                { text: '🔢 Generate Temp Code', callback_data: 'generate_link_code' },
                { text: '⌨️ Enter Temp Code', callback_data: 'enter_link_code' }
            ],
            [
                { text: '❓ Help', callback_data: 'link_help' }
            ]
        ]
    };
...
// Handle email + key connection initiation
async function handleConnectEmailKey(chatId, userId) {
    // Clear any existing sessions
    userSessions.delete(`${userId}_email`);
    userSessions.delete(`${userId}_awaiting_telegram_key`);
    userSessions.delete(`${userId}_awaiting_code`);
    
    // Start email input process
    userSessions.set(`${userId}_awaiting_email`, true);
    
    const message = `
🔐 *Connect with Email \\+ Telegram Key*

**Step 1 of 2:** Enter your email address

Please type the email address you used to register in the Kita\\-kita app:

📧 *Example:* yourname@email\\.com

⌨️ *Waiting for your email\\.\\.\\.*
    `;
    
    await bot.sendMessage(chatId, message, { parse_mode: 'MarkdownV2' });
}

// Handle code entry (waiting for user to type code)  
...
💡 *Valid formats:*
• **6\\-character code** from the Kita\\-kita app \\(e\\.g\\., ABC123\\)
• **Registration key** from sign\\-up \\(e\\.g\\., TG\\-XXXXX\\-XXXXX\\)

Just type and send your code\\. I'll link your account automatically\\!
...
async function processLinkCode(chatId, userId, code) {
    try {
        ...
        if (!codeDoc.exists) {
            await bot.sendMessage(chatId, '❌ Invalid or expired link code. Please check the code and try again.');
            return;
        }
        ...
        if (codeData.used) {
            await bot.sendMessage(chatId, '❌ This link code has already been used. Please generate a new one in the app.');
            return;
        }
        
        // Check if code is expired
        if (codeData.expiresAt && codeData.expiresAt.toDate() < new Date()) {
            await bot.sendMessage(chatId, '❌ This link code has expired. Please generate a new one in the app.');
            return;
        }
        ...
        const linkData = {
            ...
            linkMethod: 'link_code'
        };
        
        // Store the link in both collections for easy lookup
        ...
        // Clear awaiting session
        userSessions.delete(`${userId}_awaiting_code`);
        
        const successMessage = `
✅ *Account Successfully Linked\\!*

🎉 Your Telegram account is now connected to your Kita\\-kita app\\!

*What you can now do:*
📸 Send receipts to automatically sync with your app
💰 Get personalized AI financial advice  
📊 View balances and transactions
🤖 Access all AI agents with your real data

Send a receipt photo or choose an AI agent to get started\\!
        `;
        ...
        await bot.sendMessage(chatId, '❌ Failed to process link code. Please try again or contact support.');
    }
}

// Handle special telegram keys from user registration
async function processTelegramKey(chatId, userId, telegramKey) {
    try {
        // Look up the telegram key in Firestore
        const keyDoc = await db.collection('telegram_keys').doc(telegramKey).get();
        
        if (!keyDoc.exists) {
            await bot.sendMessage(chatId, '❌ Invalid telegram key. Please check your key from the Kita-kita app and try again.');
            return;
        }
        ...
        if (keyData.used) {
            await bot.sendMessage(chatId, '❌ This telegram key has already been used. Please regenerate a new one in the app if needed.');
            return;
        }
        
        // Check if key is expired
        if (keyData.expiresAt && keyData.expiresAt.toDate() < new Date()) {
            await bot.sendMessage(chatId, '❌ This telegram key has expired. Please regenerate a new one in the app.');
            return;
        }
        ...
        const linkData = {
            ...
            linkMethod: 'telegram_key'
        };
        
        // Store the link in both collections for easy lookup
        await db.collection('telegram_links').doc(keyData.userId).set(linkData);
        await db.collection('telegram_links').doc(userId.toString()).set(linkData);
        
        // Mark the key as used
        await db.collection('telegram_keys').doc(telegramKey).update({
            used: true,
            usedAt: admin.firestore.FieldValue.serverTimestamp(),
            telegramUserId: userId.toString()
        });
        
        // Update user record to mark telegram key as used
        await db.collection('users').doc(keyData.userId).update({
            telegramKeyUsed: true,
            telegramLinkedAt: admin.firestore.FieldValue.serverTimestamp(),
            telegramUserId: userId.toString()
        });
        
        // Clear awaiting session if it exists
        userSessions.delete(`${userId}_awaiting_code`);
        
        const successMessage = `
🎉 *Welcome to Kita\\-kita\\!*

✅ Your account has been successfully linked using your registration key\\!

*Your account is now fully connected:*
📱 App ↔️ Telegram Bot synchronization active
🤖 AI agents have access to your real financial data
📸 Receipt scanning will auto\\-sync to your app
💰 Get personalized financial insights

*Quick Start:*
📸 Send me a receipt photo to test it out
💰 Try the Ipon Coach for savings advice
🛡️ Use Gastos Guardian to analyze spending
...
Welcome aboard\\! 🚀
        ...
        console.log(`🎉 Successfully linked new user ${keyData.userId} (Telegram: ${userId}) using registration key`);
        
    } catch (error) {
        console.error('❌ Error processing telegram key:', error);
        await bot.sendMessage(chatId, '❌ Failed to process telegram key. Please try again or contact support.');
    }
}

// Handle email and telegram key verification
async function processEmailAndTelegramKey(chatId, userId, email, telegramKey) {
    try {
        // Call the server endpoint to verify credentials
        const response = await fetch(`${process.env.SERVER_URL || 'http://localhost:3000'}/api/telegram/verify-credentials`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok || !result.success) {
            await bot.sendMessage(chatId, `❌ Authentication failed: ${result.reason || result.error}`);
            
            // Clear sessions and offer to try again
            userSessions.delete(`${userId}_email`);
            userSessions.delete(`${userId}_awaiting_telegram_key`);
...
Use /link to start the connection process again, or contact support if you're having trouble with your credentials.
            `;
            
            await bot.sendMessage(chatId, retryMessage, { parse_mode: 'MarkdownV2' });
            return;
        }

        // Successful verification - create the link
        ...
        // Store the link in both collections for easy lookup
        await db.collection('telegram_links').doc(userData.userId).set(linkData);
        await db.collection('telegram_links').doc(userId.toString()).set(linkData);
        
        // Mark the telegram key as used
        await db.collection('telegram_keys').doc(telegramKey).update({
            used: true,
            usedAt: admin.firestore.FieldValue.serverTimestamp(),
            telegramUserId: userId.toString()
...
✅ **${userData.userData.name || userData.email}** \\- Your Kita\\-kita account is now connected\\!

*Your account is now fully integrated:*
📱 App ↔️ Telegram Bot synchronization active
🤖 AI agents have access to your real financial data
📸 Receipt scanning will auto\\-sync to your app
💰 Get personalized financial insights based on your data

*What you can do now:*
📸 Send me a receipt photo to scan and categorize
💰 Chat with Ipon Coach for personalized savings advice
🛡️ Use Gastos Guardian to analyze your spending patterns
🗺️ Get financial planning help from Pera Planner

Welcome aboard\\! Send a receipt or choose an AI agent to get started\\! 🚀
        `;
        
        await bot.sendMessage(chatId, successMessage, { parse_mode: 'MarkdownV2' });
        
        console.log(`🎉 Successfully linked user ${userData.userId} (${userData.email}) to Telegram user ${userId}`);
        
    } catch (error) {
        console.error('❌ Error processing email and telegram key:', error);
        await bot.sendMessage(chatId, '❌ Failed to verify credentials. Please try again or contact support.');
